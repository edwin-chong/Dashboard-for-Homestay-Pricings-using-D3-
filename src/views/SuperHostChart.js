...
function SuperHostChart(props) {
  ...
  const draw = (d) => {
    ...
    const aggregatedData = d3.group(tmpData, (d) => d.host_is_superhost); // Extract month from date
    ...
    const data = Array.from(aggregatedData, ([t, avg_price]) => ({
      is_superhost: t === "t" ? "Superhost" : "Non-Superhost",
      avg_price: +avg_price,
    }));
  ...
  return (
    <div className="pane">
      <div className="header">Super Host vs Avg Price</div>
